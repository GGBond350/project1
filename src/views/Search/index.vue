				...
				<div class="bread">
					...
					<ul class="fl sui-tag">
						...
						<li class="with-x" v-if="searchParams.categoryName">{{ searchParams.categoryName }}<i
								@click="RCategoryName">×</i></li>
						<!-- keywords -->
						<li class="with-x" v-if="searchParams.keywords">{{ searchParams.keywords }}<i @click="RKeywords">×</i></li>
						<!-- 商标 -->
						<li class="with-x" v-if="searchParams.trademark">{{ searchParams.trademark.split(':')[1] }}<i
								@click="RTrademark">×</i></li>
						<!-- 属性 -->
						<li class="with-x" v-for="(attr, index) in searchParams.props" :key="index">{{ attr.split(':')[1] }}<i
								@click="RProps(index)">×</i></li>
					</ul>
				</div>

				<!--selector-->
				<SearchSelector @receiveTrademark="hanlderTrademark" @receiveAttr="handlerAttr" />
						...
						<ul class="yui3-g">
							...
							<li class="yui3-u-1-5" v-for="(good) in goodsList" :key="good.id">
								<router-link :to="`/detail/${good.id}`">
									<div class="list-wrap">
										<div class="p-img">
											<a href="item.html" target="_blank"><img v-lazy="good.defaultImg" /></a>
										</div>
										<div class="price">
											<strong>
												<em>¥ </em>
...
export default {
	...
	methods: {
		...
		change(tag) {
			if (tag === 1) {
				if (this.isOne) {
					this.isDown ? (this.searchParams.order = '1:asc') : (this.searchParams.order = '1:desc')
				...
				if (this.isTwo) {
					this.isDown ? (this.searchParams.order = '2:asc') : (this.searchParams.order = '2:desc')
				} else if (this.isOne) {
	...
	computed: {
		...
		isOne() {
			return this.searchParams.order.startsWith('1')
		},
		isTwo() {
			return this.searchParams.order.startsWith('2')
		},
		isUp() {
