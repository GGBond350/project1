	...
	<div class="order-right">
		...
		<div class="order-content">
				...
				<table class="order-item" v-for="(order, index) in records" :key="order.id">
					...
					<tbody>
						<tr v-for="(item, index) in order.orderDetailList" :key="item.id">
							...
							<td :rowspan="order.orderDetailList.length" width="8%" class="center" v-if="index === 0">{{ order.consignee
								}}</td>
			...
			<div class="choose-order">
				<!-- 分页器 -->
				<Pagination :pageNo='page' :pageSize="limit" :total="data.total" :continues="5" @changePage="changePage">
				</Pagination>
			</div>
		</div>
		<!--猜你喜欢-->
...
export default {
	...
	methods: {
		async getData() {
			const res = await this.$API.reqMyorderList(this.page, this.limit)
			if (res.code === 200) {
				this.data = res.data
			}
		},
